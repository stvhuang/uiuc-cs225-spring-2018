  ...
  public:
    ...
    class Iterator : std::iterator<std::forward_iterator_tag,
      ...
      public:
         ...
         */
        Iterator() : traversal(NULL) { /*Nothing*/ }
         ...
         */
        Iterator(TreeTraversal<T> &traversal,
         ...
         */
        typename BinaryTree<T>::Node *operator*() { return current; }
        ...
        bool operator!=(const Iterator &other) {
            ...
            if (thisEmpty && otherEmpty)
                return false;  // both empty then the traversals are equal,
                               // return true
            else if ((!thisEmpty) && (!otherEmpty))
                return (traversal != other.traversal);  // both not empty then
                                                        // compare the traversals
            else
                return true;  // one is empty while the other is not, return
                              // true
        }
