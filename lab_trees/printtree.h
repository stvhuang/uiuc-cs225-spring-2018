...
template <class NodeDescriptor>
void printSubtree(NodeDescriptor croot, std::vector<std::string> &output,
                  int left, int top, int curr_width);
}  // namespace private_print_functions
...
void printTree(NodeDescriptor root, std::ostream &out = std::cout) {
    ...
    for (size_t i = 0; i < output.size(); ++i)
        output[i] = std::string(print_matrix_width + 4,
                                ' ');  // + 4 extra room for long things
...
void printSubtree(NodeDescriptor croot, std::vector<std::string> &output,
    ...
    for (size_t i = 0; i < nodeStr.length() &&
                       left + curr_width / 2 + i < output[top].length();
         i++)
    ...
    if (!croot.right().isNull()) {
        ...
        for (int pos = center + left_start_shift + nodeStr.length() + 1;
             pos < branch_pos; pos++)
            output[top][pos] = '_';
        // draw right '\'
        output[top + 1][branch_pos] = '\\';
        ...
        for (int pos = branch_pos + 1; pos < rightcenter; pos++)
            output[top + 1][pos] = '_';
        // draw right subtree
        printSubtree(croot.right(), output, left + curr_width / 2 + 2, top + 2,
                     curr_width / 2 - 1);
    }
}
}  // namespace private_print_functions
