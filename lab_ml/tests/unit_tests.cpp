...
bool edgeExists(const Graph &g, const std::string &label1,
...
TEST_CASE("NimLearner(10) constructor creates a starting vertex \"p1-10\"",
          "[weight=1][part=1]") {
    NimLearner nim(10);
...
TEST_CASE("NimLearner(3) constructor creates the correct vertices",
          "[weight=1][part=1]") {
    NimLearner nim(3);
    ...
    // REQUIRE( findVertexLabel(g, "p2-3") ); okay but not required, never
    // reached
...
TEST_CASE("NimLearner(3) constructor creates the correct edges",
          "[weight=1][part=1]") {
    NimLearner nim(3);
...
TEST_CASE("Path from playRandomGame of NimLearner(10) source vertex is labeled "
          "\"p1-10\"",
          "[weight=1][part=2]") {
    NimLearner nim(10);
...
TEST_CASE("Path from playRandomGame of NimLearner(10) final vertex is labeled "
          "\"p1-0\" or \"p2-0\"",
          "[weight=1][part=2]") {
    NimLearner nim(10);
...
TEST_CASE("A NimLearner(1) random path contains one edge",
          "[weight=1][part=3]") {
    NimLearner nim(1);
...
TEST_CASE("NimLearner(1) updates the edge weight correctly",
          "[weight=1][part=3]") {
    NimLearner nim(1);
    std::vector<Edge> path = nim.playRandomGame();
    Edge e = path[0];
...
TEST_CASE(
    "NimLearner(3) learns that (p1-3, p2-2) and (p1-3, p2-1) are losing edges",
    "[weight=1][part=3]") {
    NimLearner nim(3);
    ...
    const Graph &g = nim.getGraph();
    REQUIRE(g.getEdgeWeight(g.getVertexByLabel("p1-3"),
                            g.getVertexByLabel("p2-2")) >
            g.getEdgeWeight(g.getVertexByLabel("p1-3"),
                            g.getVertexByLabel("p2-1")));
}

TEST_CASE(
    "NimLearner(4) learns that (p1-4, p2-3) is a better edge than (p1-4, p2-2)",
    "[weight=1][part=3]") {
    NimLearner nim(4);
    ...
    const Graph &g = nim.getGraph();
    REQUIRE(g.getEdgeWeight(g.getVertexByLabel("p1-4"),
                            g.getVertexByLabel("p2-3")) >
            g.getEdgeWeight(g.getVertexByLabel("p1-4"),
                            g.getVertexByLabel("p2-2")));
}

TEST_CASE(
    "NimLearner(4) has ending edge weights summing to the random games played",
    "[weight=1][part=3]") {
    NimLearner nim(4);
    ...
    REQUIRE(g.getEdgeWeight(p1_2, p2_0) + g.getEdgeWeight(p1_1, p2_0) +
                g.getEdgeWeight(p2_2, p1_0) + g.getEdgeWeight(p2_1, p1_0) ==
            10000);
}
