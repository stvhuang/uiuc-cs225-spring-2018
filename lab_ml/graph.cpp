...
Vertex Graph::removeVertex(Vertex v) {
    if (adjacency_list.find(v) != adjacency_list.end()) {
        if (!directed) {
            for (auto it = adjacency_list[v].begin();
...
Vertex Graph::insertVertexByLabel(string label) {
    ...
    removeVertex(label);
    // make it empty again
    adjacency_list[label] = unordered_map<Vertex, Edge>();
    vertex_label_map[label] = label;
    return label;
}

Vertex Graph::getVertexByLabel(string label) const {
    for (const Vertex &v : getVertices()) {
        if (getVertexLabel(v) == label)
            return v;
    }
...
void Graph::print() const {
    for (auto it = adjacency_list.begin(); it != adjacency_list.end(); ++it) {
        ...
        for (auto it2 = it->second.begin(); it2 != it->second.end(); ++it2) {
            ...
            string edgeColumn =
                "edge label = \"" + it2->second.getLabel() + "\"";
...
void Graph::savePNG(string title) const {
    ...
    if (result == 0) {
        ...
        cout << "Failed to generate visual output graph using `neato`. Install "
                "`graphviz` or `neato` to generate a visual graph."
             << endl;
    }
