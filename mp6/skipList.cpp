...
void SkipList::insert(int key, HSLAPixel value) {
    ...
    // If the SkipNode exists, the key exists in our list and the value for that
    // key must be updated to the new value.
    if (temp) {
        temp->value = value;
        return;
    }
 ...
 * HSLAPixel Returns (0, 0, 0, 0.5) if it's not found
...
SkipNode *SkipList::findI(int key) {
    ...
    while (traverse->nodePointers[0].next != tail_ && level >= 0) {
        ...
        if (nextKey == key) {
            ...
            traverse = traverse->nodePointers[level].next;
            // level=traverse->nodePointers.size()-1; // slightly slows down
            // code in our sorted inserts
        }
    }
    return NULL;
}
 ...
 * pointers.
 * @return a vector containing the keys from head_ to tail_, including the
 * sentinel values
 */
