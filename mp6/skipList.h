...
class SkipList {
  public:
    ...
    /**
     * Constructs the SkipList with one initial node, which is constructed with
     * the provided values.
     *
     ...
     * @return A boolean indicating whether a node was successfully removed from
     * the list
     */
    bool remove(int key);
     ...
     */
    PNG toPNG(int maxHeight = 100, HSLAPixel fg = HSLAPixel(0, 0, 0),
              HSLAPixel bg = HSLAPixel(0, 0, 1)) const;
  ...
  private:
    ...
    /**
     * Finds the given key in the list if it exists, and returns a pointer to
    ...
    /**
     * Finds the given key in the list if it exists, and returns a pointer to
     * the node containing it. Randomly calls findR or findI
     * @param key The key to search for in the list
     * @return A pointer to the node containing key, or NULL if not found in the
     * list
     */
    ...
    /**
     * A function that will get the current node's next and previous node's keys
     * at each level Used because STL containers don't play well with gdb.
    ...
    /**
     * A function that will print out the current node's next and previous
     * node's keys at each level Used because STL containers don't play well
     * with gdb. Note that this doesn't work well if you are using layour src
