 ...
 * @param imgSize A power of two which gives how large of an image this list
 * represents
 */
...
/*
 * Returns whether the given value is a power of two, using the magic of
 * bitshifting
...
HSLAPixel colorPicker(string s, HSLAPixel d = HSLAPixel(0, 0, 0)) {
    ...
    if (s == "black") {
        color = HSLAPixel(0, 1.0, 0.0);
    } else if (s == "red") {
        color = HSLAPixel(0, 1.0, 0.5);
    } else if (s == "green") {
        color = HSLAPixel(120, 1.0, 0.5);
    } else if (s == "blue") {
        color = HSLAPixel(240, 1.0, 0.5);
    } else if (s == "white") {
        color = HSLAPixel(0, 1.0, 1.0);
    } else {
        cout << "Invalid color. Please choose one of the following: 'black', "
...
int main(int argc, char *argv[]) {
    // Setting seed for random number generator use time(NULL) for more random
    // behavior Seed it for deterministic numbers, useful for debugging
    // srand(225);
    srand(time(NULL));

    if (argc < 3 || argc > 5) {
        cout << argv[0]
             << " <image_size> <probability> [foreground color] [background "
                "color]"
             << endl;
    ...
    if (!powerOf2(img_size) || img_size < 4 || img_size > 256) {
        cout << "Please enter your an image size greater than 2 and less than "
                "512 "
                "and as a power of 2."
             << endl;
        return 0;
    ...
    if (probability > 100 || probability < 1) {
        cout << "`probability` must be an integer between 1 and 100 (You used "
             << probability << ")." << endl;
        return 0;
    }

    if (img_size >= 256) {
        cout << "Using large image size! out-structure.png will be generated "
                "but not viewable in the default editor!"
                " Use something like chrome to view (google-chrome "
                "out-structure.png &)"
             << endl;
    }
    ...
    SkipList *test =
        new SkipList(probability, maxLevelLog(img_size * img_size));
    ...
    cout << "Value of search(4) "
         << (test->search(4) == temp ? "matches" : "doesn't match")
