...
#define READ_SOLUTION_PNG(func, width, height)  \
        read_solution(string("tests/soln_") + func + string(".png"), width,
height)
#define READ_UNSOLVED_PNG(func, width, height)  \
        read_unsolved(string("tests/soln_") + func + string(".png"), width,
height)
...
void recDFS(SquareMaze & maze, vector<vector<uint8_t> > * visited, int x, int y,
int width, int height, int * calls)
{
...
pair<int, int> assert_maze_helper(SquareMaze & maze, int width, int height)
{
        ...
        for (int i = 0; i < width; i++) {
                for (int j = 0; j < height; j++) {
                        if (visited[i][j] == 0) {
                                recDFS(maze, &visited, i, j, width, height,
&calls); components++;
                        }
                }
        }
        ...
        for (int x = 0; x < source.getWidth(); x++)
        {
                for (int y = 0; y < source.getHeight(); y++)
                {
                        if (x < source.getWidth() - 1)
                                dest->setWall(x, y, 0, source.isWall(x, y,
MazeReader::RIGHTWALL)); if (y < source.getHeight() - 1) dest->setWall(x, y, 1,
...
TEST_CASE("testMakeMazeRandom", "[weight=10][part2]")
{
  ...
  bool same = true;
        ...
        for (int y = 0; y < 50; y++)
        {
                for (int x = 0; x < 50; x++)
                {
                        if (maze1.canTravel(x, y, 0) != maze2.canTravel(x, y,
0)){ same = false; break;
      }
                        if (maze1.canTravel(x, y, 1) != maze2.canTravel(x, y,
1)){ same = false; break;
      }
                }
        }
...
{
        SquareMaze maze;
        MazeReader soln = READ_SOLUTION_MAZE("testSolutionCorrectSquare",15,
15); copyMaze(soln, &maze); vector<int> solution = maze.solveMaze();
...
void helpSolveMaze(const MazeReader & soln)
{
        ...
        for (size_t i = 0; i < solution.size() && i < soln.getSolutionSize() &&
i < 10; i++) cout << "step " << i << ": actual=" << solution[i] << ", expected="
<< soln.getSolutionAt(i) << endl;
