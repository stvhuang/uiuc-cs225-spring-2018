...
size_t insertion_idx_time(vector<int> *vec) {
    // check needed since monad calls this with a vector of size 1 (to ``warm
    // up'' the generator/timing function)
    if (vec->size() <= 2) {
        size_t val = 0;
...
TEST_CASE("test_btree12_strings", "[weight=5]") {
    vector<pair<string, string>> data = {
        {"lls", "skye"},        {"mjoras", "yogi"},    {"collin", "meco"},
        {"fred", "skye"},       {"arya", "nymeria"},   {"jon", "ghost"},
        {"bran", "summer"},     {"robb", "grey wind"}, {"sansa", "lady"},
        {"rickon", "shaggydog"}};
