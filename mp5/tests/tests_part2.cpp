...
TEST_CASE("Creates a basic MosaicCanvas (gridtest)", "[weight=5][part=2]") {
    ...
    PNG a(1, 1);
    a.getPixel(0, 0) = HSLAPixel(0, 1, 0.5);  // red
    PNG b(1, 1);
    b.getPixel(0, 0) = HSLAPixel(120, 1, 0.5);  // green
    PNG c(1, 1);
    c.getPixel(0, 0) = HSLAPixel(240, 1, 0.5);  // blue
    ...
    PNG expected;
    expected.readFromFile("tests/gridtest-expected.png");
