...
void compareBinaryFiles(string yourFile, string ourFile) {
    ...
    if (!tree_equals_output(ours, yourFile))
        FAIL("Your output in \"" + yourFile +
             "\" does not match our output in \"" + ourFile + "\"");
}
...
TEST_CASE("KDTree::shouldReplace Tests", "[weight=1][part=1]") {
    ...
    REQUIRE(tree.shouldReplace(target, currentBest3, possibleBest3) ==
            false);  // operator<
}
...
void _test_linear_constructor(int size) {
    ...
    compareBinaryFiles(fname,
                       "tests/expected_kdtree_" + to_string(size) + ".kd");
}
...
TEST_CASE("KDTree::findNearestNeighbor, exact match, 1D (Dim=1)",
          "[weight=1][part=1]") {
    _test_linear_nearestNeighbor<1>(10);
}

TEST_CASE("KDTree::findNearestNeighbor, exact match, 3D (Dim=3)",
          "[weight=1][part=1]") {
...
TEST_CASE("KDTree::findNearestNeighbor (2D), returns correct result",
          "[weight=1][part=1]") {
    double coords[6][2] = {{-15, 7}, {6, 7},   {-13, -1},
                           {-5, 0},  {14, -3}, {14, 2}};
...
class MineActionFAIL : public Point<Dim>::MineAction {
  public:
    void onMine(const Point<Dim> &point) const {
        ...
        if (trigger)
            FAIL("The point " << point
                              << " should not be visited during this query.");
    }
...
// N: nearest neighbor
// X: non-mine elements
TEST_CASE("KDTree::findNearestNeighbor (2D), testing correct path",
          "[weight=1][part=1]") {
    double coords[10][2] = {
        {-8, 7},
        {-6, 4},
...
*/
TEST_CASE(
    "KDTree::findNearestNeighbor (2D), testing correct path with fence jumping",
    "[weight=1][part=1]") {
    double coords[20][2] = {{84, 44},                      // mine
                            ...
                            {34, 15},                      // mine
                            {42, 63}, {96, 56},            // mine
                            {44, 79}, {44, 43}, {28, 10},  // mine
                            {60, 30},                      // mine
                            {88, 72},                      // mine
                            {75, 68},                      // mine
                            {43, 65}, {48, 0},             // mine
                            {14, 15},                      // mine
                            {49, 83}, {51, 35}, {95, 50},  // mine
                            {82, 20}};                     // mine
...
TEST_CASE("KDTree::findNearestNeighbor (3D), testing tie-breaking",
          "[weight=1][part=1]") {
    double coords[14][3] = {{0, 0, 100},                             // mine
                            ...
                            {0, 0, 50},                              // mine
                            {100, 0, 100},                           // mine
                            {100, 100, 0}, {0, 0, 0},   {0, 50, 0},  // mine
                            {50, 0, 0},                              // mine
                            {50, 0, 50},                             // mine
                            {100, 0, 0},                             // mine
                            {50, 50, 0},   {0, 100, 0},              // mine
                            {50, 50, 50}};
...
TEST_CASE("KDTree::findNearestNeighbor (3D), testing that left recursion does "
