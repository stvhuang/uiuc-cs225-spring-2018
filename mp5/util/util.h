...
struct Initializer {
    T value;
    Initializer()
        : value()  // ====> default construction, works for classes _and_
                   // built-in
    {}
};
...
class OptionsParser {
  ...
  public:
    OptionsParser();
    void addOption(const string &name, bool &setValue) {
        optsMap[name] = &setValue;
    }
...
// EXEC()
int8_t exec(int redirect_fd, const char *command, const char *arg1 = NULL,
            const char *arg2 = NULL, const char *arg3 = NULL,
            const char *arg4 = NULL, const char *arg5 = NULL,
            const char *arg6 = NULL);

inline int8_t exec(const char *command, const char *arg1 = NULL,
                   const char *arg2 = NULL, const char *arg3 = NULL,
                   const char *arg4 = NULL, const char *arg5 = NULL,
                   const char *arg6 = NULL) {
...
inline int8_t exec(const string &command, const string &arg1) {
    return exec(command.c_str(), arg1.c_str());
}
inline int8_t exec(const string &command, const string &arg1,
                   const string &arg2) {
    return exec(command.c_str(), arg1.c_str(), arg2.c_str());
}
inline int8_t exec(const string &command, const string &arg1,
                   const string &arg2, const string &arg3) {
    return exec(command.c_str(), arg1.c_str(), arg2.c_str(), arg3.c_str());
}
inline int8_t exec(const string &command, const string &arg1,
                   const string &arg2, const string &arg3, const string &arg4) {
    return exec(command.c_str(), arg1.c_str(), arg2.c_str(), arg3.c_str(),
                arg4.c_str());
}
inline int8_t exec(const string &command, const string &arg1,
...
inline int8_t exec(int redirect_fd, const string &command) {
    return exec(redirect_fd, command.c_str());
}
inline int8_t exec(int redirect_fd, const string &command, const string &arg1) {
    return exec(redirect_fd, command.c_str(), arg1.c_str());
}
inline int8_t exec(int redirect_fd, const string &command, const string &arg1,
                   const string &arg2) {
    return exec(redirect_fd, command.c_str(), arg1.c_str(), arg2.c_str());
}
inline int8_t exec(int redirect_fd, const string &command, const string &arg1,
                   const string &arg2, const string &arg3) {
    return exec(redirect_fd, command.c_str(), arg1.c_str(), arg2.c_str(),
...
inline int8_t exec(int redirect_fd, const string &command, const string &arg1,
                   ...
                   const string &arg5, const string &arg6) {
    return exec(redirect_fd, command.c_str(), arg1.c_str(), arg2.c_str(),
                arg3.c_str(), arg4.c_str(), arg5.c_str(), arg6.c_str());
}
...
inline int8_t exec(const string &command, const vector<string> &args) {
    return exec(command, args, &std::cout);
}
...
string operator+(const string &lhs,
                 const string &rhs);  // These 5 functions are to disambiguate
string operator+(const string &lhs,
                 const char *rhs);  // operator+ so that the following
...
template <typename T>
class enable_if<false, T> {};  // No ::type here
...
#define EXIT_IF_ERROR(...)                                                     \
    EXIT_IF_ERROR_THIRD_ARG(__VA_ARGS__, EXIT_IF_ERROR2, EXIT_IF_ERROR1, 0)    \
    (__VA_ARGS__)
...
inline ssize_t write(int fd, int val) { return writen(fd, &val, sizeof val); }

inline ssize_t write(int fd, long val) { return writen(fd, &val, sizeof val); }

inline ssize_t read(int fd, int &val) { return readn(fd, &val, sizeof val); }

inline ssize_t read(int fd, long &val) { return readn(fd, &val, sizeof val); }
