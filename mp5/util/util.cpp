...
void freeCstr(const char *cstr) { delete[] cstr; }
...
pair<int, int> getFdsFromStream(const ostream *stream) {
    ...
    if (stream == NULL)
        redirect_fd = -1;
             ...
             stream == &cerr)  // we don't handle cerr right now; it is just
                               // left unpiped
...
exec_timers get_exec_timers() {
    ...
    if (getitimer(ITIMER_VIRTUAL, &result.remaining_virtual) != 0) {
        if (errno == EINVAL) {
            ...
            exit_if_error_output(__FILE__, __LINE__,
                                 "getitimer(ITIMER_VIRTUAL) failed");
    }
...
void set_exec_timers(const exec_timers &timers) {
    ...
    if (timers.supports_virtual)
        EXIT_IF_ERROR(
            setitimer(ITIMER_VIRTUAL, &timers.remaining_virtual, NULL));
    if (timers.supports_prof)
        EXIT_IF_ERROR(setitimer(ITIMER_PROF, &timers.remaining_prof, NULL));
}
...
char **cstr_array_from_vector_string(const string &command,
...
void fd_to_ostream(int fd, ostream *output) {
    ...
    do {
        ...
        if (char_read_count < 0 && errno == EINTR)
            continue;
 ...
 */
pid_t exec_cmd(const char *cmd, const char *const *args, int redirect_fd,
               int outpipe_fd = -1) {
    // Save timer values :)
    ...
    if (pid == 0) {
        ...
        char path[] =
            "PATH=/class/cs225/llvm/bin:/bin/:/usr/bin:/usr/local/bin";
        char libpath[] =
            "LD_LIBRARY_PATH=/software/intel-composer-2011u5-x86_64/lib/"
            "intel64/:$LD_LIBRARY_PATH";  // path for libiomp5.so
        char licenses[] =
            "INTEL_LICENSE_FILE=28518@intel.webstore.illinois.edu:/software/"
            "intel-composer-2011u5-x86_64/composerxe-2011.5.220/licenses:/opt/"
            "intel/licenses:/home/massung1/intel/licenses";  // also for
                                                             // parallel labs
        ...
        char *newenv[] = {path,           libpath,      licenses,
                          redirect_glibc, asan_options, NULL};
        environ = newenv;
...
int monad_system(const char *command) { return exec("sh", "-c", command); }

int8_t exec(const string &command, const vector<string> &args,
            ostream *output) {
...
// originally from:
// http://stackoverflow.com/questions/2180079/how-can-i-copy-a-file-on-unix-using-c
int8_t exec(int redirect_fd, const string &command,
            const vector<string> &args) {
    using namespace internal::exec;

    // avoid self destruction errors from closing then trying to duplicate
    // output you can't redirect to what's already there
...
int8_t exec(int redirect_fd, const char *command, const char *arg1,
...
int chdir(const string &dir) { return ::chdir(dir.c_str()); }
...
void copyFiles(const string &sourceFolder, const string &destFolder,
...
void protectDir(const string &dir) {
    (void)dir;
    // (debug) EXIT_IF_ERROR(exec("/bin/chmod", "-R", "ugoa-w", dir.c_str()) !=
    // 0);
}
...
vector<string> get_files_in_dir(const string &dir,
                                bool concatdir /* = true */) {
    EXIT_IF_ERROR(dir == "" || dir[dir.length() - 1] != '/',
                  "Directory name must end in a '/'");
...
string get_symlink_target(const string &symlink) {
    ...
    EXIT_IF_ERROR(len < 0 || static_cast<size_t>(len) == buf_size,
                  "Error getting target of symlink " + symlink);
    buf[len] = '\0';
    return string(buf);
}

void linkDirs(const string &sourceFolder, const string &destFolder,
...
size_t replaceAllInternal(string &str, const string &toreplace,
                          const string &with) {
    size_t i = str.find(toreplace);
    size_t count = 0;
...
vector<string> OptionsParser::parse(const vector<string> &rawArgs) {
    ...
    for (size_t arg_i = 1; arg_i < rawArgs.size(); arg_i++) {
        ...
        if (currarg.compare(0, 2, "--") == 0)  // long option
        {
            ...
            string value = (equalspos >= currarg.length())
                               ? ""
                               : currarg.substr(equalspos);
        ...
        else if (currarg[0] == '-')  // string of single char options
        {
            for (size_t c = 1; currarg[c] != '\0'; c++) {
                optsMap_t::iterator option =
                    optsMap.find(string(1, currarg[c]));
                if (option == optsMap.end()) {
...
char *processOptions(int argc, char **argv, OptionsMap &opts,
                     vector<string> &args) {
    for (int arg_i = 1; arg_i < argc; arg_i++) {
        ...
        if (strncmp(currarg, "--", 2) == 0)  // long option
        {
            ...
            if (valuechar == 'o')
                valuechar = tolower(currarg[equals_i + 1]) + 1;
            switch (valuechar) {
            case '\0':
                value = true;
                break;
            case 'f' + 1:
                value = false;
                break;  // off: see 'o' above
            case 'n':
                value = false;
                break;
            case 'n' + 1:
                value = true;
                break;  // on: contrast 'n': see 'o' above
            case 't':
                value = true;
                break;
            case 'y':
 ...
 **/
ssize_t writeBytesToFile(signed int fileDescriptor, const char *buffer,
...
void rename_main(const string &file, const string &newname) {
    ...
    exec("sed", "-i", "-e",
         ("s/int[\\r\\n \\t][\\r\\n \\t]*main[\\r\\n \\t]*(\\(.*\\))/namespace "
          "proxy_test { int " +
          newname + "(\\1); } int proxy_test::" + newname + "(\\1)/")
             .c_str(),
         file.c_str());
}
...
vector<string> tokenize(const string &str, const string &delims) {
    ...
    size_t start = 0;
    while (start < str.length() && delims.find(str[start]) != string::npos)
        start++;
    size_t end;
    for (end = str.find_first_of(delims); end != string::npos;
