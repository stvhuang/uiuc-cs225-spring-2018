...
TEST_CASE("Image lighten() does not lighten a pixel above 1.0",
          "[weight=1][part=1]") {
    Image img = createRainbowImage();
...
TEST_CASE("Image darken(0.2) does not darken a pixel below 0.0",
          "[weight=1][part=1]") {
    Image img = createRainbowImage();
...
TEST_CASE("Image rotateColor(double) keeps the hue in the range [0, 360]",
          "[weight=1][part=1]") {
    Image img = createRainbowImage();
...
// scale
//
TEST_CASE("Image scale(2.0) results in the correct width/height",
          "[weight=1][part=1]") {
    Image img = createRainbowImage();
...
TEST_CASE("Image scale(0.5) results in the correct width/height",
          "[weight=1][part=1]") {
    Image img = createRainbowImage();
...
TEST_CASE("Image scale(2) scales pixel data in a reasonable way",
          "[weight=1][part=1]") {
    Image img = createRainbowImage();
...
TEST_CASE("Image scale(0.5) scales pixel data in a reasonable way",
          "[weight=1][part=1]") {
    Image img = createRainbowImage();
