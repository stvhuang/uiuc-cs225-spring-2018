...
TEST_CASE("createSpotlight", "[weight=1]") {
    ...
    SECTION("Spotlight creates an 80% dark pixel >160 pixels away") {
        REQUIRE(png.getPixel(320, 50).l * 0.2 ==
                Approx(result.getPixel(320, 50).l));
    }

    SECTION("Spotlight is correct at 20 pixels away from center") {
        REQUIRE(png.getPixel(100, 50 + 20).l * 0.9 ==
                result.getPixel(100, 50 + 20).l);
    }

    SECTION("Spotlight is correct at 5 pixels away from center") {
        REQUIRE(png.getPixel(100 + 3, 50 + 4).l * 0.975 ==
                result.getPixel(100 + 3, 50 + 4).l);
    }
}
...
TEST_CASE("illinify", "[weight=1]") {
    ...
    SECTION("After illinify, image contains only orange and blue hues") {
        for (unsigned x = 0; x < result.width(); x++) {
            for (unsigned y = 0; y < result.height(); y++) {
                if (result.getPixel(x, y).h != 11 &&
                    result.getPixel(x, y).h != 216) {
                    FAIL("The hue of (" << x << "," << y << ") is "
                                        << result.getPixel(x, y).h
    ...
    SECTION("Hue wrap-arounds are correct (remember: h=359 is closer to orange "
            "than blue)") {
