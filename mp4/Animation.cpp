...
void Animation::addFrame(PNG const &img) { frames.push_back(img); }

PNG Animation::getFrame(unsigned index) { return frames[index]; }

unsigned Animation::frameCount() { return frames.size(); }
...
void Animation::write(const std::string &filename) {
    ...
    system(
        ("ls frames | grep '^" + name + ".*\\.png$' | xargs -I% rm -f frames/%")
            .c_str());

    // Generate Frames
    for (size_t i = 0; i < frames.size(); i++)
        frames[i].writeToFile(
            ("frames/" + name + getString(i, frames.size()) + ".png").c_str());

    // Combine frames
    system(("convert frames/" + name + "*.png " + filename).c_str());

    // To .mp4
    // system(("ffmpeg -f 1 -i frames/" + name + "%02d.png -vcodec libx264 -r 30
    // -pix_fmt yuv420p " + filename + ".mp4").c_str());
}
